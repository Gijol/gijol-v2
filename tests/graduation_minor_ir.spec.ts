...
describe('manual-backed intelligent robot minor requirements', () => {
  ...
  it('does not treat arbitrary AI courses as designated IR courses', async () => {
    const result = await evaluateFor(
      2026,
      [
        course({ courseCode: 'AI3001' }),
        course({ courseCode: 'AI3003' }),
        course({ courseCode: 'AI2601' }),
        course({ courseCode: 'AI3601' }),
      ],
      { userMinors: ['IR'], minorDeclarationTerms: { IR: { year: 2026, semester: '1' } } },
    );
    expectRequirement(result, 'minor-credits-IR', { acquiredCredits: 6, satisfied: false });
    expectRequirement(result, 'minor-history-review-IR', { status: 'needs_review' });
  });
