...
describe('calcAverageGrade', () => {
  ...
  it('should exclude S/U courses from the grade average', () => {
    expect(
      calcAverageGrade([
        { credit: 3, grade: 'A0' },
        { credit: 3, grade: 'S' },
  ...
  it('should not lose an exact hundredth to floating-point noise', () => {
    expect(truncateGradeAverage(2.2999999999999998)).toBe(2.3);
  });

  it('should calculate a major average by matching classified course codes to transcript grades', () => {
    const transcript = [
      { courseCode: 'EC3101', credit: 3, grade: 'A+' },
      { courseCode: 'EC4204', credit: 3, grade: 'B0' },
      { courseCode: 'GS1607', credit: 2, grade: 'A0' },
    ];

    expect(calcAverageGradeForCourseCodes(transcript, ['EC3101', 'EC4204'])).toBe(3.75);
  });
