...
function buildSingleCategory(
  ...
  return {
    messages,
    minConditionCredits: minConditionCredits || 0,
    satisfied,
    totalCredits,
...
export function calculateGradStatus(body: GradStatusRequestBody): GradStatusResponseType {
  const { takenCourses = [], userMajor, userMinors } = body;

  // ✅ 최소 학점 기준 (핸드북에 맞게 필요 시 조정)
  const mins = {
    languageBasic: 7, // 영어+글쓰기 7학점 (2021+ 기준)
    scienceBasic: 17,
    humanities: 24, // 인문사회 24학점
    etcMandatory: 8, // 새내기+전공탐색+논문연구 등 합산 최소 추정
    major: 36, // 전공필수 36 (전공 전체 36~42)
    minor: 0, // 부전공은 선택: 나중에 부전공 선언 시 18 등으로 조정 가능
  };
  ...
  // ✅ 카테고리별 상태 계산 (한 번만)
  const languageBasicCat = buildSingleCategory(
    'languageBasic',
    grouped.languageBasic,
    mins.languageBasic
  );
  const scienceBasicCat = buildSingleCategory(
    'scienceBasic',
    grouped.scienceBasic,
    mins.scienceBasic
  );
  const majorCat = buildSingleCategory('major', grouped.major, mins.major);
  const minorCat = buildSingleCategory('minor', grouped.minor, mins.minor);
  const humanitiesCat = buildSingleCategory('humanities', grouped.humanities, mins.humanities);
  const etcMandatoryCat = buildSingleCategory(
    'etcMandatory',
    grouped.etcMandatory,
    mins.etcMandatory
  );
  const otherCat = buildSingleCategory('otherUncheckedClass', grouped.otherUncheckedClass, 0);

  const graduationCategory = {
    languageBasic: languageBasicCat,
    scienceBasic: scienceBasicCat,
    major: majorCat,
    minor: minorCat,
    humanities: humanitiesCat,
    etcMandatory: etcMandatoryCat,
    otherUncheckedClass: otherCat,
  };
  ...
  const requiredSatisfied =
    languageBasicCat.satisfied &&
    scienceBasicCat.satisfied &&
    majorCat.satisfied &&
    humanitiesCat.satisfied &&
    etcMandatoryCat.satisfied;

  const totalSatisfied = requiredSatisfied && totalCredits >= TOTAL_REQUIRED_CREDITS;

  const grad: GradStatusResponseType = {
    graduationCategory,
    totalCredits,
    totalSatisfied,
  };
