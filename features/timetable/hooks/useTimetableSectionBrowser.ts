...
export async function fetchTimetableSectionsByCourseCodes(
  ...
  do {
    const response = await fetchTimetableSectionPage(term, '', '', page, signal, courseCodes);
...
export function useTimetableSectionBrowser(term: string, query: string, department: string) {
  const result = useInfiniteQuery({
    queryKey: ['timetable-section-browser', term, query, department],
    queryFn: ({ pageParam = 1, signal }) => fetchTimetableSectionPage(term, query, department, pageParam, signal),
