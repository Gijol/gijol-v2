...
export function TimetableHome({ defaultTerm, timetableSources }: TimetableHomeProps) {
  ...
  const representativePlan = currentGroup?.representativePlanId ? plans[currentGroup.representativePlanId] : undefined;
  const representativeSelectedCount = representativePlan ? getSelectedSectionCount(representativePlan) : 0;
  ...
  return (
    <div className="min-h-full overflow-y-auto bg-slate-100 p-4 lg:p-8">
      <div className="mx-auto flex max-w-7xl flex-col gap-6">
        <header className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
          <div>
            <p className="text-xs font-black tracking-[0.2em] text-blue-600 uppercase">Timetable</p>
            <h1 className="mt-1 text-2xl font-black tracking-tight text-slate-950 sm:text-3xl">시간표</h1>
            <p className="mt-2 text-sm font-medium text-slate-500">
              앞으로 계획할 시간표와 성적표 기반 이전 시간표를 나눠 관리합니다.
            </p>
            <div className="mt-3 flex flex-wrap gap-2">
              <Badge variant="outline" className="bg-white text-slate-600">
                {formatCourseTerm(currentTerm)}
              </Badge>
              <Badge variant="outline" className="bg-white text-slate-600">
                시간표 {currentPlans.length}개
              </Badge>
              {representativePlan && (
                <Badge variant="outline" className="bg-white text-slate-600">
                  대표 {representativeSelectedCount}분반
                </Badge>
              )}
              {legacyEntries.length > 0 && (
                <Badge variant="outline" className="bg-white text-slate-600">
                  기존 계획 {legacyEntries.length}개
                </Badge>
              )}
            </div>
          </div>
          <Button onClick={createNewPlan} className="h-10 bg-blue-600 px-4 font-bold hover:bg-blue-700">
            <Plus size={18} className="mr-2" />새 시간표
          </Button>
        </header>
            ...
            <div className="min-w-0">
              <div className="flex items-center gap-2">
                ...
                <h2 className="truncate text-lg font-black tracking-tight text-slate-950">
                  {formatCourseTerm(currentTerm)} 계획 시간표
                </h2>
              </div>
          ...
          <div className="grid gap-3 p-4 sm:grid-cols-2 xl:grid-cols-3">
            ...
            >
              <div>
                ...
                <p className="mt-1 text-sm font-medium text-slate-500">현재 학기에 맞춰 빈 계획을 시작합니다.</p>
              </div>
              <span className="inline-flex items-center text-xs font-black text-blue-600">
                만들기
              ...
              return (
                ...
                >
                  ...
                  <div className="mt-4 min-w-0">
                    <h3 className="truncate text-base font-black tracking-tight text-slate-950" title={plan.name}>
                      {plan.name}
                    </h3>
                    <p className="mt-1 truncate text-xs font-medium text-slate-400">{formatDate(plan.updatedAt)}</p>
                  </div>

                  <div className="mt-4 grid grid-cols-2 gap-2 text-xs">
                    <div className="rounded-md bg-slate-50 px-3 py-2">
                      <div className="font-black text-slate-950">{selectedCount}개</div>
                      <div className="mt-0.5 font-medium text-slate-500">선택 분반</div>
                    </div>
                    <div className="rounded-md bg-slate-50 px-3 py-2">
                      <div className="font-black text-slate-950">{creditCount}학점</div>
            ...
            {currentPlans.length === 0 && (
              <div className="flex min-h-[188px] items-center justify-center rounded-lg border border-slate-200 bg-white p-4 text-center">
                <div>
                  <p className="text-sm font-black text-slate-500">아직 저장된 시간표가 없습니다.</p>
                  <p className="mt-1 text-xs font-bold text-slate-400">왼쪽의 새 시간표 타일로 시작하세요.</p>
                </div>
              </div>
            )}
          </div>
        ...
        <section className="rounded-lg border border-slate-200 bg-white p-5 shadow-sm">
          <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
            <div className="flex min-w-0 items-start gap-3">
              ...
              <div className="min-w-0">
                <h2 className="text-lg font-black tracking-tight text-slate-950">이전 시간표</h2>
                <p className="mt-1 text-sm font-medium text-slate-500">
            ...
            <Button
              ...
              onClick={() => {
                void router.push('/dashboard/graduation/upload');
              }}
            >
          ...
          {previousPlans.length > 0 && (
            <div className="mt-5">
              <div className="mb-2 text-xs font-black tracking-widest text-slate-400 uppercase">생성된 이전 시간표</div>
                ...
                {previousPlans.map((plan) => (
                  <div key={plan.id} className="flex min-w-0 items-center gap-3 rounded-md border border-slate-200 p-3">
                    ...
                    <div className="min-w-0 flex-1">
                      <div className="truncate text-sm font-extrabold text-slate-900">{plan.name}</div>
          ...
          <div className="mt-5">
            <div className="mb-2 text-xs font-black tracking-widest text-slate-400 uppercase">성적표에서 생성</div>
            {completedTermSummaries.length === 0 ? (
              <div className="rounded-lg border border-dashed border-slate-200 bg-slate-50 p-5 text-center">
                <p className="text-sm font-black text-slate-500">업로드된 성적표 과목이 없습니다.</p>
                ...
                {completedTermSummaries.map((summary) => {
                  ...
                  return (
                    <div key={summary.key} className="rounded-md border border-slate-200 p-3">
                      <div className="flex items-start justify-between gap-3">
                        <div className="min-w-0">
                          <div className="truncate text-sm font-extrabold text-slate-900">{summary.label}</div>
                      ...
                      {errorMessage && (
                        <div className="mt-2 flex items-start gap-1.5 text-xs font-medium text-red-500">
        ...
        {legacyEntries.length > 0 && (
          <section className="rounded-lg border border-slate-200 bg-white p-5 shadow-sm">
            <div className="flex items-center gap-2">
              <FolderClock size={18} className="text-amber-500" />
              <h2 className="text-lg font-black tracking-tight text-slate-950">학기 미상 기존 계획</h2>
            </div>
            <p className="mt-1 text-sm font-medium text-slate-500">
      ...
      <Dialog open={legacyImportName !== null} onOpenChange={(open) => !open && setLegacyImportName(null)}>
        <DialogContent className="max-w-md">
          <DialogHeader>
            <DialogTitle className="text-xl font-black tracking-tight">계획 대상 학기 지정</DialogTitle>
          </DialogHeader>
